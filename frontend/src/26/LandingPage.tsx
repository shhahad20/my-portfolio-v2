...
// ─── Liquid Variable-Font Title ───────────────────────────────────────────────
const LiquidTitle: React.FC<{ lines: string[] }> = ({ lines }) => {
  const allRefs = useRef<(HTMLSpanElement | null)[]>([]);
  const allChars = lines.join("").split("");

  const handleEnter = (hoveredIndex: number) => {
    allRefs.current.forEach((el, i) => {
      if (!el) return;
      const dist = Math.abs(i - hoveredIndex);
      el.dataset.proximity = String(dist);
    });
  };

  const handleLeave = () => {
    allRefs.current.forEach((el) => {
      if (el) el.dataset.proximity = "";
    });
  };
  ...
  return (
    <h1 className="title" aria-label={lines.join(" ")}>
        ...
        <div className="landing__left">
          ...
          >
            SHAHAD <br /> ALTHARWA
                    ...
                    <LiquidTitle lines={["SHAHAD", "ALTHARWA"]} />
